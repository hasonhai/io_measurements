...
\begin{abstract}
A typical instruction for every administrator when deploy big data application is: do not virtualize your cluster. But cloud is the trending now and avoiding virtualizing the cluster is sometimes impossible for small vendors or also for big vendors when they want to use the same infrastructure for every team in their organization. The reason for a ``No-no`` is that Big data application utilises data locality to improve performance while cloud often use a remote storage. In order to keep data locality, the VM instances was setup to opererate on local hard drive and it create a situation in which many concurrently processes competing for IO resources with an intensive data volume. We designed a synthesis big data workload and do the experiments to compare between virtualized platform and baremetal. We found that virtualization overhead is impossible to avoid. However, we can reduce the impacts by selecting the optimized I/O scheduler, the optimized flavors for the VM instances, and a good``write-cache`` technique.
